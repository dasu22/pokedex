...
let sortCol: keyof Pokemon = "number";
let sortDir: 1 | -1 = 1;
...
const regionEl    = document.getElementById("filter-region")  as HTMLSelectElement;
const typeEl      = document.getElementById("filter-type")    as HTMLSelectElement;
const caughtEl    = document.getElementById("filter-caught")  as HTMLSelectElement;
const resetBtn    = document.getElementById("btn-reset-filters")!;

[searchEl, regionEl, typeEl, caughtEl].forEach(el =>
...
document.querySelectorAll<HTMLTableCellElement>("th.sortable").forEach(th => {
  th.addEventListener("click", () => {
    const col = th.dataset.col as keyof Pokemon;
    if (sortCol === col) {
...
function populateFilters(): void {
  ...
  const types = [...new Set(
    allPokemon.flatMap(p => [p.type1, p.type2].filter(Boolean))
  )].sort();
  types.forEach(t => {
    const opt = document.createElement("option");
    opt.value = t;
...
function renderTable(): void {
  ...
  let filtered = allPokemon.filter(p => {
    ...
    if (caught === "caught"  && !p.caught) return false;
    if (caught === "missing" &&  p.caught) return false;
    return true;
  });
  ...
  tbody.querySelectorAll<HTMLButtonElement>(".catch-btn").forEach(btn => {
...
// ── Toggle caught ──────────────────────────────────────────────────────
async function toggleCaught(id: number, btn: HTMLButtonElement): Promise<void> {
  btn.classList.add("loading");
  try {
    const res = await fetch(`/api/pokemon/${id}/toggle`, { method: "POST" });
    if (!res.ok) throw new Error("Toggle failed");
    const updated: Pokemon = await res.json();
